...
def Cost_Function(setP, P, ax, ay):
    ...
    '''
    cost = np.ravel([[F(P, ax[i])-setP[i][0], F(P, ay[i])-setP[i][1]]
...
def Jacobian_Soloff(P, ax, ay):
    ...
    '''
    jac = [[[dFdx(P, ax[i]), dFdy(P, ax[i]), dFdz(P, ax[i])], [
        dFdx(P, ay[i]), dFdy(P, ay[i]), dFdz(P, ay[i])]] for i in np.arange(len(ax))]
...
def NewtonSoloff_Triangulation(setP, ax, ay, params):
    ...
    setP, aX, aY = setP[foundSetPoints[:, 0]], np.asarray(
        ax)[foundSetPoints[:, 0]], np.asarray(ay)[foundSetPoints[:, 0]]
    P = np.array([(params.Vmax[0]+params.Vmin[0])/2, (params.Vmax[1] +
                 params.Vmin[1])/2, (params.Vmax[2]+params.Vmin[2])/2])
    for i in range(5):
        P += np.linalg.lstsq(Jacobian_Soloff(P, aX, aY), -
                             Cost_Function(setP, P, aX, aY), rcond=None)[0]
    costsP = np.linalg.norm(Cost_Function(setP, P, aX, aY).reshape(
        len(aX), 2), axis=1)  # cost per cam
    return P, costsP
...
def NewtonSoloff_Extend(setP, P_predict, aX, aY):
    ...
    for i in range(5):
        P[:3:] += np.linalg.lstsq(Jacobian_Soloff(P[:3:], aX, aY), -
                                  Cost_Function(setP, P[:3:], aX, aY), rcond=None)[0]
    P[-1] = np.mean(np.linalg.norm(Cost_Function(setP,
                    P[:3:], aX, aY).reshape(2*len(aX), 1)))
    return P
