...
def create_target_points(targetname, npointsX, npointsY, spacing, zOffset):
    ...
    np.savetxt(f"calibration_targets/{filename}.csv", points,
               fmt='%.3f', delimiter=",", header="x,y,z", comments="")


def create_z_planes(zmin: float, zmax: float, nplanes: int):
    return np.linspace(zmin, zmax, nplanes)
